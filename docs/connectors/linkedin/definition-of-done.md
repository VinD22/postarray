# Connector Definition of Done — LinkedIn
...
| Field | Value |
| --- | --- |
| Provider | LinkedIn, via the official LinkedIn Posts API (Community Management) |
| Connector version | `1.0.0` (`identity().connectorVersion`, `packages/connectors/src/providers/linkedin/connector.ts`) |
| Contract version implemented | `1.0.0` (`CONNECTOR_CONTRACT_VERSION`, `packages/connectors/src/contract.ts`) |
| Engineering owner | _(unfilled — see section 12: `identity()` records `'Backend/Connectors 2'`, a team label, not a named individual)_ |
...
None of these seven items can be satisfied by code. Every one needs a live LinkedIn Developer
Portal application, a human decision, or both.
...
- [ ] The provider's official API documentation URL and official policy URL are recorded in the
      ...
      _Needs:_ no `docs/connectors/linkedin/runbook.md` exists (see section 11). `README.md` in the
...
- [ ] Production application status is recorded and is one of: approved; approved with a named
      ...
      _Needs:_ `README.md` states Community Management app review is "Not started as of 4 August
      2026," calling it "the longest lead review of the six," target Week 3. A human must confirm
      current status and attach evidence.
...
- [ ] The scopes we request are the minimum for shipped features. Every requested scope maps to a
      ...
      _Needs:_ `authorization()` in `connector.ts` (lines ~503-557) already names a screen
      (`usedBy`) for every one of the six requested scopes, and reading the rest of the file
      confirms each is genuinely exercised (`w_member_social`/`w_organization_social` in
      `publish`, `openid`/`profile` in `discoverAccounts`, `r_organization_social` in
      `fetchMetrics`, `rw_organization_admin` in `discoverAccounts`'s organization-ACL lookup and
      `listDestinations`). That proves internal consistency, but not that the scopes actually
      configured on the live LinkedIn Developer Portal application match this list — that is a
      live-config fact code cannot see. A reviewer must confirm the two match.
- [ ] Login OAuth and publisher OAuth use separate applications and separate credentials.
      ...
      `LINKEDIN_CLIENT_ID`/`LINKEDIN_CLIENT_SECRET` pair, used only by `providers.linkedin` (the
      publishing connector). No separate "Sign in with LinkedIn" login application was found
      anywhere in `apps/`. If LinkedIn is only ever used as a publishing connector and never as an
      account login method, this item may not apply — but that is a product decision, not
...
- [x] `discoverAccounts` returns every eligible external identity and the user chooses explicitly.
      ...
      _Evidence:_ `packages/connectors/src/providers/linkedin/connector.test.ts`, describe
      `LinkedIn account discovery`, tests `returns the member plus only the organizations with a
      publishing role` (asserts 3 accounts returned, including an ineligible one with
      `page_role_required`, none silently dropped) and `still connects the member when the
      organization list is unavailable` (both passing). `discoverAccounts` in `connector.ts` is a
      pure read (`GET /v2/userinfo` plus an optional `GET /rest/organizationAcls`) — it performs no
      connection-creating side effect, only returns `ExternalAccount[]`. Whether the connections UI
      actually renders that array as an explicit picker rather than auto-connecting is an
      `apps/web` concern this test does not reach, but the connector itself cannot and does not
      auto-connect anything.
- [ ] Connecting the same external account twice in one workspace is prevented by the
      ...
      _Needs:_ `packages/database` schema/constraint concern, not connector code.
...
- [ ] Disconnect calls the provider revoke endpoint where one exists, deletes our stored credential
      ...
      _Needs:_ `connector.ts` `revoke()` does call `POST /oauth/v2/revoke` and does log-and-continue
      rather than throw on a provider failure (matching the "delete our credential regardless"
      intent), but no test in `linkedin/connector.test.ts` or elsewhere calls
      `connector.revoke()` at all. This is unproven, not just unchecked for a documentation reason.
...
- [ ] Token refresh runs at 75% of lifetime, stores the rotated refresh token atomically with the
      ...
      declares `refreshAtLifetimeFraction: 0.75`. But no test calls
      `connector.refreshCredential()` for LinkedIn at all, so atomic storage of the rotated pair
      and the failure-escalation path are unproven for this connector specifically.
- [x] Credentials are stored through the envelope-encrypted vault with AAD binding
      ...
      and `credentialKind` uniformly for every connector, LinkedIn included, so this evidence
...
- [ ] No token appears in any Temporal workflow history. Evidence: the automated history scan test.
      _Needs:_ no test with this description was found anywhere in the repository. This evidence
      does not yet exist for any connector, LinkedIn included.
...
- [x] `getCapabilities` returns a schema-valid `CapabilitySnapshot` including every field, with
      ...
      `$provider connector satisfies the contract` for `provider: 'linkedin'`, test `returns a
      schema valid capability snapshot` (passing), backed by `buildLinkedInCapabilities` degrading
      fields honestly by account type and granted scope
      (`linkedin/connector.test.ts`, `LinkedIn capability snapshot` →
      `is honest that member post read back is restricted for new applications`).
      **Terminology note for the reviewer:** the actual `CapabilitySupport` enum in
      ...
      LinkedIn's capability snapshot uses `requires_review` everywhere this item's prose says
      `requires_permission` (e.g. `analytics.support` for a member connection,
      `destinations[0].support`, `firstComment.support` pending Community Management approval).
      This looks like a naming drift between this gate document and the implemented contract that
      a human should reconcile, not a functional gap — the four-state honesty the item is really
      asking for is present.
- [ ] The three comment capabilities are declared **separately and truthfully**:
      ...
      `comment_count: 'supported'`, `comment_replies: 'not_implemented'`) and `README.md`'s "The
      three comment capabilities" table, and `capabilities.ts` further conditions `firstComment` on
      `reviewApproved('linkedin')` (currently false, so it is actually `requires_review` in a live
      snapshot despite `identity()` saying `supported` — worth a reviewer's attention, since those
      two are inconsistent with each other). Either way, the item explicitly requires a reviewer
      who is not the implementer to independently re-confirm against LinkedIn's current official
      docs — code cannot supply that.
- [ ] No limit used by the product is hard-coded outside `packages/connectors`. Evidence: the lint
      rule passes.
      _Needs:_ no such lint rule was found anywhere in the repository.
...
- [ ] A capability that changed between approval and dispatch stops the publish with
      ...
      passing) — but it is provider-agnostic (built from a fake snapshot, not LinkedIn's), and the
      observed outcome names are `CAPABILITY_UNSUPPORTED` / `CONNECTION_ACTION_REQUIRED` as
      `RelayError` codes and `validation_needed` / `action_required` as workflow states, not
      literally the six-class `USER_ACTION_REQUIRED` string this item names. A reviewer should
      confirm the naming maps the way this item assumes before checking it off.
- [ ] `validateDraft` produces a typed issue with a stable code, a user-safe message key and the
      ...
      `linkedin/connector.test.ts` `LinkedIn validation` → `requires a title on a document post`
      (`DOCUMENT_TITLE_REQUIRED`) and `names the page role remediation when the organization scope
      is missing` (`ORGANIZATION_ROLE_REQUIRED`) — but "one test per limit" is a claim about every
      field in the capability snapshot (image count, video duration, byte ceilings per media kind,
      the carousel minimum, and more), and no such exhaustive set exists for LinkedIn (for example
      `CAROUSEL_NEEDS_TWO_IMAGES`, implemented in `connector.ts`, has no test).
...
- [ ] Where the provider requires a user decision (for example a TikTok privacy selection), an
      ...
      _Needs:_ LinkedIn's visibility selector has an explicit, code-declared safe default
      (`privacy.mustBeExplicit: false`, `options` list marks `PUBLIC` as `isDefault: true` in
      `capabilities.ts`), which is the opposite of what this item is warning against for a
      TikTok-style forced choice. Whether a safe default is the right policy call for LinkedIn
      specifically (rather than requiring an explicit choice) is a product decision, not a code
      fact, so this is left unchecked rather than marked N/A.
...
- [ ] Status confirmation is implemented: `getStatus` returns `processing`, `published`, `failed`
      ...
      _Needs:_ `getStatus` in `connector.ts` implements all four states (404→`failed`,
      `lifecycleState !== 'PUBLISHED'`→`processing`, otherwise→`published`, no-id→`unknown`), and
      the generic contract suite exercises it with schema validation
      (`contract.test.ts`, `returns a schema valid publish result and status` for `linkedin`). But
      `linkedin/connector.test.ts` has **no** `getStatus` test at all — none of the four states are
      directly exercised by a LinkedIn-specific test.
- [ ] Idempotency: the provider's idempotency mechanism is used where one exists. Where it does
      ...
      _Needs:_ this is a genuine, specific finding, not just missing evidence. `identity()` declares
      `provider_idempotency: 'unsupported'` — LinkedIn has no idempotency token per this
      connector's own self-description. Yet `connector.ts` `publish()` sends
      `'x-restli-method': 'create'` on every post creation with a comment calling it "LinkedIn's
      own idempotency control on post creation," and performs **no** pre-create existence query
      (no equivalent of X's `findRecentMatchingPost`) before calling `POST /rest/posts`. Either the
      code comment is wrong (that header is not an idempotency mechanism — it only specifies the
      REST.li method type) and a preflight is missing, or `provider_idempotency` should say
      `supported` and the header genuinely dedupes retries on LinkedIn's side. Nothing in this
      repository proves either reading, and no test exercises a retry-after-timeout scenario for
      LinkedIn the way `x/connector.test.ts`'s `adopts an existing post instead of creating a
      duplicate` does for X. This should be resolved before LinkedIn is trusted with an
      automatic-retry publish path.
- [ ] Duplicate-publication chaos tests pass: worker crash after the provider accepted, provider
      ...
      `createLinkedInConnector`. Given the open idempotency question directly above, this item is
      especially important to close for LinkedIn before relying on retries.
- [ ] Partial success is handled honestly: a failed thread part or first comment leaves the root
      ...
      _Needs:_ `connector.ts` `publish()` implements this pattern (a failed
      `POST /rest/socialActions/{urn}/comments` is pushed to `failures` while the root post's
      `PublishItemResult` stays in `items`, exactly mirroring X's logic), but no test in
      `linkedin/connector.test.ts` exercises a failed-comment scenario the way X's `keeps the root
      published when a thread part fails` does. Implemented, not proven.
...
- [ ] Media preparation is idempotent on `(asset, connection, variant)`; a retry does not re-upload.
      _Needs:_ concrete gap, not just missing evidence:
      `providers/linkedin/connector.ts` sets `reusedFromPreviousAttempt: false` unconditionally in
      all three of `prepareImage` (line 240), `prepareDocument` (line 283) and `prepareVideo`
      (line 394) — the field is never derived from an actual dedup check anywhere in this
      connector.
...
receipt-write path is idempotent, but it does not touch LinkedIn, and it does not prove the full
field list this section requires.
...
- [x] Every provider error observed in fixtures and in the simulator classifies into exactly one of
      ...
      six classes (`classifiedProviderErrorSchema` constrains the field), proven generically in
      `errors.test.ts`, describe `the taxonomy`, test `has exactly the six classes from the
      handoff` (passing). Demonstrated against LinkedIn's own fixture specifically:
      `linkedin/connector.test.ts` `maps a missing page role to a connection action, not a generic
      failure` (LinkedIn's real 403 `LINKEDIN_ROLE_MISSING_FIXTURE` shape →
      `CONNECTION_ACTION_REQUIRED`, thrown explicitly by `connector.ts` rather than falling through
      to the generic classifier — worth a reviewer's note that this bypasses
      `classifyProviderError` entirely for this one status code).
- [x] Only known-safe transient operations retry. Nothing else retries automatically.
      ...
      defaults any unrecognised operation name to `'other'` — so LinkedIn's actual operation
      strings (`linkedin.create_post`, `linkedin.create_comment`, etc., none of which are in the
      safe `PROVIDER_OPERATIONS` allowlist) are non-retryable by default, not by omission.
- [x] Every classified error maps to a remediation code with user-facing copy and, where possible,
      ...
      Proves every `REMEDIATION_CODES` entry, including `page_role_required` (the one LinkedIn
      throws directly), has a `messageKey`, an `actionKey`, an `errorCode` and
      `showsInActionCenter: true`. It does **not** prove that `packages/i18n` actually defines
      translated copy for every one of those keys.
...
- [x] No provider payload, token, internal ID or another tenant's data can appear in a user-facing
      ...
      simulator scenario.
      _Evidence:_ exactly this scenario is tested — `sanitize.test.ts`, describe `sanitizeText`,
      test `removes a bearer token echoed back by a provider` (passing), and `errors.test.ts`,
      describe `classifyProviderError`, test `strips a token the provider echoed into the error
      body` (passing). Both match "Provider echoes a bearer token in an error body" in
      `docs/planning/05-social-connectors.md` section 5.3 verbatim. The sanitizer is applied
      uniformly by `classifyProviderError`, which LinkedIn's `providerFailure`/`ensureOk` calls
      route through (with the one direct-throw exception noted above, which never reaches the
      sanitizer at all and should be checked separately for payload leakage).
- [ ] Remediation appears in the Action Center, not only as a toast.
      ...
      but no test confirms the Action Center UI (`apps/web`) actually renders from it. Data
      contract proven; UI rendering unverified.
...
- [ ] Every metric field we display is documented with the provider's field name, the provider's
      ...
      `metric_definitions`.
      _Needs:_ no `metric_definitions` artifact was found anywhere in the repository.
      `providers/linkedin/metrics.ts` documents field mappings in comments and a
      `MetricFieldMapping` structure, which is close in spirit but is not the named artifact.
...
- [x] A metric the provider does not return is `unavailable` with a reason. It is never `0` and
      ...
      _Evidence:_ `linkedin/connector.test.ts`, describe `LinkedIn metrics`, test `reports member
      post impressions as unavailable because LinkedIn restricts them` (passing) — asserts
      `impressions.value === null` and `availability: 'unavailable_permission'` for a member
      connection, while `likes` (which the social actions endpoint does return) still reports a
      real value in the same call. `fetchMetrics` in `connector.ts` routes every non-2xx or
      not-applicable case through `mapMetrics` with an explicit `missingAvailability` reason.
- [x] Where the provider restricts deriving or combining data (YouTube in particular), we do not
      ...
      _Evidence:_ `providers/linkedin/metrics.ts` maps every field 1:1 (`providerField` →
      `normalizedName`, no arithmetic across fields), and `fetchMetrics` in `connector.ts` passes
      `parsed.likesSummary`/`commentsSummary`/`totalShareStatistics` straight into `mapMetrics`
      with no derived computation anywhere. This item names YouTube specifically as the sharpest
      case, but the same discipline holds for LinkedIn by inspection of the only places metric
      values are produced.
- [x] Where analytics are genuinely not available for this connector, the connector declares
      ...
      _Evidence:_ `capabilities.ts`, `buildLinkedInCapabilities`: `analytics.support` is explicitly
      `requires_review` for a member connection and only `supported` for an organization
      connection with `r_organization_social` granted — never silently blank. Proven directly by
      `linkedin/connector.test.ts`, `is honest that member post read back is restricted for new
      applications`. (Same `requires_review` / `requires_permission` naming note as section 3
      applies here.)
...
- [ ] Observed rate limits and quota are recorded in `provider_limits` with reset hints.
      _Needs:_ `provider_limits` is a database table concern. `capabilities.ts` deliberately sets
      `rateLimit: null` for LinkedIn (application and member daily limits are only visible in the
      LinkedIn Developer Portal, per the code comment and `README.md`), so this item depends
      entirely on the runtime-observed-limit recording path, which is outside this connector's
      code.
- [x] Backoff uses exponential delay with jitter and respects any provider reset hint.
      ...
      `bounds the backoff and honours a provider reset hint` (passing). Every connector, LinkedIn
      included, issues requests through this same `ProviderHttpClient`, so the guarantee applies
      directly and uniformly, not by analogy.
...
- [ ] For metered providers, a cost estimate is shown in the composer, in the schedule confirmation
      ...
      _Not applicable as worded:_ LinkedIn is not a metered provider —
      `capabilities.ts` declares `cost: null` and `identity()` declares
      `metered_cost: 'unsupported'`. There is no per-post cost to estimate or reconcile.
- [ ] For X specifically: the estimate distinguishes a plain post create from a post create
      ...
      _Not applicable:_ this item is X-specific by its own text and has no LinkedIn analogue in the
      code (`metered_cost: 'unsupported'`).
- [ ] Quota exhaustion produces `Retry scheduled` with an honest next-window time, not `Failed`.
      ...
      generically (section 6), but the actual `Retry scheduled` surface state is a worker/UI
      concern.
...
- [ ] The shared deterministic preflight runs for this connector: duplicate fingerprint,
      ...
      `packages/connectors/src/providers/linkedin`. Not found or verified in this review's scope.
- [ ] This provider's specific operating rules from `docs/research/05-trust-safety-and-legal.md`
      ...
      _Needs:_ no `docs/connectors/linkedin/runbook.md` exists (section 11).
- [ ] Required disclosures are implemented: AI or altered-content declaration, commercial content
      ...
      _Needs:_ concrete gap: `capabilities.ts` declares all three disclosure fields
      (`aiLabel`, `commercialContent`, `brandedContent`) `'unsupported'` — meaning either LinkedIn
      does not offer these, or (per the `README.md` open-questions list, which does not mention
      disclosure at all) this has simply not been re-verified against LinkedIn's current API.
...
- [ ] Alt text is required or explicitly waived for image posts wherever the platform supports it.
      ...
      snapshot, { unit: 'utf16', allowMixedMedia: false })` without overriding `requireAltText`, so
      it uses the shared default of `false` (`providers/shared/validate.ts` line 26). A missing alt
      text on a LinkedIn image post is only ever a `warning`, never a blocking `error`, and there
      is no explicit "waive" affordance the way Bluesky has (`altTextWaived`). This satisfies
      neither half of the item ("required" or "explicitly waived") — it is silently optional
      today, the same gap found for X.

## 10. Testing

- [x] Recorded, redacted fixtures exist with capture date and API version, and contain no secret.
      _Evidence:_ `packages/connectors/src/providers/linkedin/__fixtures__/index.ts`, header
      comment: "Recorded LinkedIn response shapes, API version `202603`. Every id, URN and name
      below is fabricated. Retrieved 4 August 2026." Capture date and API version are both present
      in the file itself. No secret-shaped value was found in the fixture file on inspection, and
      `AGENTS.md` rule 2 ("No secrets in source... including tests and fixtures") applies
      repository-wide, not just where a file comments on it.
- [ ] A provider simulator exists and can produce every scenario in
      `docs/planning/05-social-connectors.md` section 5.3.
      _Needs:_ a simulator exists (`providers/shared/testing.ts`) and its `ScriptedRoute` mechanism
      can technically produce any of the 14 listed scenarios. Confirmed present for LinkedIn today
      are the happy path, a missing-page-role rejection, and a create with no `x-restli-id` header
      (LinkedIn's own "malformed response" case). Not found wired to LinkedIn specifically: 429
      with/without a reset hint, retry-after-timeout, container/processing stuck past a timeout,
      mid-flow token expiry, capability drift at dispatch, and partial-success (comment rejected).
      Partial, not full, coverage.
- [x] The shared connector contract suite passes **unmodified** against both fixtures and the
      ...
      includes `provider: 'linkedin'` (`contract.harness.ts`, `CONTRACT_HARNESS_PROVIDERS`), and
      every one of its nine generic assertions passed when the full suite was run in this review
      (`pnpm --filter @relay/connectors test`, 500/500 passing). The suite is identical for every
      provider in the harness — LinkedIn does not get a special case.
...
- [ ] Temporal replay tests pass for every workflow this connector participates in.
      ...
      parametrized per connector, so it cannot be cited as LinkedIn-specific evidence.
...
- [ ] A connector runbook exists at `docs/connectors/<provider>/runbook.md` containing: official
      ...
      one exists.
      _Needs:_ `docs/connectors/linkedin/runbook.md` does not exist.
      `packages/connectors/src/providers/linkedin/README.md` covers some of the same ground (docs
      table, scopes table, rate-limit note) but is not the file this item names, and does not cover
      failure modes, canary procedure, or an escalation contact.
...
- [ ] The public capability page entry is generated from the versioned connector metadata and has
      ...
      what the live connector registry (including LinkedIn) actually reports, and this passed as
      part of the full suite run. The "manually reviewed before publication" half is a human step
      this cannot prove.
...
- [ ] A named engineering owner is assigned and recorded in `identity()`.
      _Needs:_ `identity()` in `connector.ts` records `engineeringOwner: 'Backend/Connectors 2'` —
...
- [ ] A next review date is set. Default cadence: every 90 days, and immediately on any provider
      ...
      default cadence this item specifies, and the same value X's connector carries. Worth flagging
      for the reviewer to either correct the constant or record a dated exception.
- [ ] A source-register row exists for every provider claim this connector relies on, with URL,
      ...
      _Needs:_ `docs/research/06-source-register.md` has an "## LinkedIn" section with four URLs,
      but its table only has `Source | Type | Used for` columns — no retrieved-date, version,
      owner, or next-review-date column is populated for any row, despite the document's own
      "Recheck schedule" section describing that as the required shape.
