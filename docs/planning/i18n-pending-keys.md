...
| `web.comparison.state.*` | Per comparison pages (C6) | The four verdict words a comparison cell can carry: yes, no, partly and not verified. A cell renders the word next to its icon, never the icon alone, so these are accessibility copy as much as interface copy. |
| `web.comparison.label.*` / `web.comparison.table.*` | Per comparison pages (C6) | Row header, source read date, the two review dates, the back link, and the table title and caption. The caption names the rule the table follows: one claim per row, with the source behind each answer. |
| `web.comparison.bestFor.*` | Per comparison pages (C6) | The section that says who each option actually suits. The alternative name is an ICU argument, never part of the string. |
| `web.comparison.notDo.*` / `web.comparison.disclosure.*` | Per comparison pages (C6) | What this product does not do, with counts read at render time from the connector capability data, the locale review records and the undecided pricing tiers. The three plural messages state that nothing publishes, that no language has completed review and that two tiers cannot be bought. A translation must not soften any of the three. |
| `web.comparison.notVerified.*` | Per comparison pages (C6) | What a not verified cell means: the fact could not be read on the other option's own documentation on the day of the check, and is never guessed. Must be translated by a person. |
| `web.comparison.method.*` | Per comparison pages (C6) | How a comparison page is made and the review cadence: at least every 90 days, and immediately when a platform changes something a row states. |
| `web.comparison.questions.*` / `web.comparison.sources.*` | Per comparison pages (C6) | Headings for the rendered questions, which are the only content eligible for FAQ structured data, and for the list of every source cited on the page. |
| `web.comparison.index.*` | Per comparison pages (C6) | The published list on `/compare`: heading, the sentence explaining why comparisons are categories rather than named products, and the per row checked date. |
| `web.blog.cluster.developers` | Blog depth (C6) | Fifth editorial cluster, for articles aimed at somebody integrating through the API rather than planning a calendar. |
| `mediaLib.derivative.*` | Media derivative pipeline (A9) | The non-generative picture editor and the version list it writes to: crop, rotate, resize, format, quality, the projected size, and the states for a version being made, one that already existed and one that could not be made. Two sentences carry the product claim and must survive translation intact. `mediaLib.derivative.originalKept` states that the original is never replaced and that each edit is a separate version. `mediaLib.derivative.nonGenerative` states that Post Array generates no images or video and that this editor only changes pixels the person already uploaded. |
| `error.media_derivative_*.message` | Media derivative pipeline (A9) | The refusals the application boundary raises before anything is enqueued: a crop outside the picture, a resize that would enlarge, an undecodable source, unknown dimensions, a missing target format, a quality setting on a lossless format, and a conversion to the format the file already is. `error.media_derivative_upscale_rejected.message` is the load bearing one: it says the product refuses to enlarge because the extra pixels would be invented rather than the person's, which is the same boundary the whole feature is built on. Every one of these sentences also states that nothing was saved and the original is untouched. |
| `web.marketing.provider.google_business_profile.*` | Public connector cohort fix | Google Business Profile is in the launch cohort but was missing from the public connector matrix, so `/integrations` listed nine of the ten. Label, account types, restriction and cost for the tenth. The restriction sentence carries the claim that no adapter is built yet and nothing publishes there today, and must survive translation intact. |
| `web.capabilities.note.noVideo` | Public connector cohort fix | Capability note for a post type that accepts text or a single image and never video. Added with the Google Business Profile record. |
| `web.meta.demo.*` | In-page product demonstration | Title and description for the guided walkthrough at `/demo`. |
| `web.demo.frame.*` / `web.demo.control.*` | In-page product demonstration | The frame every demonstration panel sits in. `web.demo.frame.sample` is the load bearing sentence and must survive translation intact: it says the panel is a demonstration built from the real interface, filled with sample content for a company that does not exist, that it is not a live account, and that nothing in it submits anything. It is the figure caption, so a screen reader reads it with the panel rather than after it. The three control labels name the demonstration in the label itself, so a pause button is never announced as a bare verb. |
| `web.demo.step.*` | In-page product demonstration | The six steps of the walkthrough: create a project, connect an account, compose and adapt, schedule or queue, watch the calendar, read the receipt. `web.demo.step.connect.body` and `web.demo.step.receipt.body` state where the workflow stops today, and must not be softened. |
| `web.demo.project.*` / `web.demo.accounts.*` / `web.demo.master.*` / `web.demo.variants.*` / `web.demo.schedule.*` / `web.demo.week.*` / `web.demo.receipt.*` | In-page product demonstration | Panel labels. `web.demo.accounts.state` says verification is not complete, and `web.demo.receipt.pending` says the publishing half of a receipt stays unavailable until a connector passes provider verification. Both are capability statements, not chrome. |
| `web.demo.sample.*` | In-page product demonstration | The sample content itself: a fictional company, its handles on the reserved `.example` domain, two first-name teammates and one release announcement. A translation should keep it recognizable as sample content rather than making it read as a real customer. |
| `web.demo.title` / `web.demo.lede` / `web.demo.notice.*` / `web.demo.contents.title` / `web.demo.stepLabel` / `web.demo.closing.*` / `web.demo.hero.*` / `web.demo.nav.*` | In-page product demonstration | Page chrome for `/demo` and the home page hero demonstration. `web.demo.notice.body` carries the prelaunch claim in the pattern the blog and platform pages established, and must not be softened in translation. |
| `web.cta.startTrial` / `web.cta.trialFootnote` / `web.home.summaryLine` / `web.home.v2.sticker.trial` / `web.meta.pricing.description` / `web.pricing.lede` / `web.pricing.prelaunch.*` / `web.pricing.perMonthNote` / `web.pricing.beside.title` / `web.pricing.beside.cancel` / `web.pricing.v2.closing.*` / `web.marketing.v2.closing.body` | Paid positioning (no free prelaunch) | No new keys. The English values changed: the product is no longer given away during prelaunch, so every string that said prelaunch access is free now states the real price ($29 a month or $300 a year, seven day trial) and says plainly that joining today is a waiting list for a paid product, not a purchase. The beta locale catalogs still carry the old free-prelaunch sentences for the keys they translate, which is now a false commercial claim in those languages. Each must be retranslated, or deleted so the reviewed English fallback shows, before any locale ships. |
| `error.post_credits_exhausted.message` / `.action` | Free plan (24 Aug) | The refusal when free publishing credits run out. Translated into every active locale; needs native review. |
| `billing.credits.*` (9 keys) | Free plan (24 Aug) | Credit balance, last-post notice, exhausted state, earn-by-referral line. English fallback until reviewed. |
| `validation.schedule_too_far_ahead.message` | 30 day schedule horizon (24 Aug) | Rewritten: the cap is now product-wide and tied to media retention. Machine-translated everywhere; needs native review. |
| `nav.public.demo`, `web.demo.hero.viewCta`, `web.demo.hero.projectsLine`, `web.demo.hero.projectsChip` | Landing demo clarity (24 Aug) | Demo nav entry, hero demo link, multi-business switcher strip. Machine-translated everywhere; needs native review. |
| Trial-copy rewrites (see `beta-fallbacks.ts`, "trial was replaced" block) | Free plan (24 Aug) | Stale trial translations deleted; keys fall back to reviewed English until each locale re-translates the free-plan story. |
