...
| D-B2 | Product name and domain (codename is "Relay") | End of week 6, 20 September 2026 | Keep `Relay` only if the trademark and the `.com` are genuinely obtainable in the software and SaaS classes. "Relay" is heavily used in developer tooling, so the default is to choose a distinct coined name by week 6 and register it. All user-visible copy already lives in `packages/i18n`, so a rename is a catalog edit plus package scope rename, roughly two days if done before week 8 and roughly two weeks if done after marketing content exists | Provider applications, legal pages, the status page, marketing content and the short-link domain all embed the name. Renaming after week 12 is expensive |
