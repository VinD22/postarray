...
const sideClasses: Record<SheetSide, string> = {
  ...
  'inline-end': 'top-0 bottom-0 end-0 h-dvh w-[min(24rem,calc(100vw-3rem))] border-s rounded-s-xl',
  'block-end': 'start-0 end-0 bottom-0 max-h-[85dvh] w-full border-t rounded-t-xl',
};
...
export function SheetHeader({ className, ...props }: ComponentPropsWithoutRef<'div'>): ReactNode {
  return (
    <div
      className={cn('border-border-subtle flex flex-col gap-1 border-b px-4 py-3 pe-12', className)}
      {...props}
    />
  );
}

export function SheetBody({ className, ...props }: ComponentPropsWithoutRef<'div'>): ReactNode {
  return (
    <div
...
export function SheetFooter({ className, ...props }: ComponentPropsWithoutRef<'div'>): ReactNode {
  return (
    <div
      className={cn(
        'border-border-subtle flex flex-col-reverse gap-2 border-t px-4 py-3 sm:flex-row sm:justify-end',
        className,
      )}
      {...props}
    />
