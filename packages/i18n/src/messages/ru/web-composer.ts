...
export const webComposerMessages = {
  ...
  'composerWeb.entity.lookupNotBuilt':
    'Post Array еще не создал поиск объектов для {provider}. Пока ничего не предполагается.',
  ...
  'composerWeb.links.domainDefault': 'Post Array домен по умолчанию',
  ...
  'composerWeb.assist.noMediaGeneration':
    'Post Array не создает изображения или видео. Готовые файлы переносите в библиотеку и публикуйте здесь.',
  ...
  'composerWeb.page.noConnectionsBody':
    'Для черновика требуется хотя бы одна подключенная учетная запись, чтобы Post Array знал ограничения, предварительный просмотр и настройки для отображения.',
  ...
  'mediaLib.empty.body':
    'Загрузите изображения и видео, которые у вас уже есть, или импортируйте файл по URL-адресу. Post Array проверяет тип и размер каждой учетной записи, в которой вы публикуете.',
  ...
  'mediaLib.import.scanPending':
    'Post Array записал источник файла. Публикация ждёт завершения проверки безопасности.',
  ...
  'mediaLib.retention.body':
    'Как только файл прикреплён к публикации, мы окончательно удаляем его из хранилища Post Array через 30 дней после создания этой публикации. Для файлов, ожидающих прикрепления, датой очистки по умолчанию служит дата загрузки. Текст публикации, квитанции о публикации и история аудита остаются доступны дольше. Публикация, уже вышедшая на социальной платформе, не удаляется, когда срок хранения её файла истекает.',
  ...
  'mediaLib.provenance.unverified':
    'Эти сведения взяты из источника, а не из Post Array. Проверьте их, прежде чем полагаться на них.',
