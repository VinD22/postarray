...
export const digestMessages = {
  ...
  'digest.empty':
    'На этой неделе пока нечего обобщать. Опубликуйте что-нибудь, и это появится здесь.',
  'digest.regenerate': 'Собрать неделю заново',
  'digest.generating': 'Составляем сводку за эту неделю',
  'digest.source.deterministic':
    'Создано по вашим записям о публикациях и собственным измерениям, без помощника письма.',
  'digest.source.ai':
    'Создано помощником по вашим записям. Каждое число проверено по этим записям.',
  'digest.unavailable.aiOff':
    'Помощник письма выключен, поэтому показана обычная версия. Ничего не пропущено.',
  'digest.unavailable.rejected':
    'Версия помощника не совпала с вашими данными и была удалена. Показана обычная версия.',
  ...
  'digest.metrics.noneYet':
    'Измерения за эту неделю ещё не поступили. Это значит, что мы не знаем результат публикаций, а не то, что он был плохим.',
  ...
  'digest.settings.description':
    'Короткое письмо каждую неделю о том, что вышло и что мы смогли измерить. Включено по умолчанию.',
