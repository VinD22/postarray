    ...
    <div className="grid min-h-dvh grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <main id="main" className="flex flex-col justify-center px-5 py-10 sm:px-8 lg:px-12">
        <div className="mx-auto w-full max-w-[26rem]">
          <p className="text-title-md text-text-primary pb-8">{intl.t.format('shell.appName')}</p>
          {children}
        </div>
      </main>

      <aside className="border-border-subtle bg-surface-sunken hidden flex-col justify-center border-s px-12 py-10 lg:flex">
        <div className="max-w-[34ch]">
          <h2 className="text-title-lg text-text-primary font-serif text-balance">
          ...
          <ul className="flex flex-col gap-4 pt-6">
            {[
              ...
              <li
                key={key}
                className="border-border-subtle text-body-md text-text-secondary border-t pt-4"
              >
                {intl.t.format(key)}
              </li>
            ))}
