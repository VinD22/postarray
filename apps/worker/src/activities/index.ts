...
import { createLogger, productMetrics, runWithContext, withSpan, type Logger } from '@relay/observability';
...
export function createActivities(deps: ActivityDependencies): WorkerActivities {
  ...
  return {
    ...
    beginPublishAttempt: wrap('beginPublishAttempt', deps, gateway.beginPublishAttempt.bind(gateway)),
    ensureNotAlreadyPublished: wrap('ensureNotAlreadyPublished', deps, gateway.ensureNotAlreadyPublished.bind(gateway)),
    ...
    publishSequenceItem: wrap('publishSequenceItem', deps, gateway.publishSequenceItem.bind(gateway)),
    ...
    scheduleAnalyticsFetches: wrap('scheduleAnalyticsFetches', deps, gateway.scheduleAnalyticsFetches.bind(gateway)),
    planRepeatOccurrence: wrap('planRepeatOccurrence', deps, gateway.planRepeatOccurrence.bind(gateway)),
    createOccurrenceJob: wrap('createOccurrenceJob', deps, gateway.createOccurrenceJob.bind(gateway)),
    fetchPostMetrics: wrap('fetchPostMetrics', deps, gateway.fetchPostMetrics.bind(gateway)),
    fetchAccountMetrics: wrap('fetchAccountMetrics', deps, gateway.fetchAccountMetrics.bind(gateway)),
    ...
    raiseConnectionIncident: wrap('raiseConnectionIncident', deps, gateway.raiseConnectionIncident.bind(gateway)),
    ...
    evaluateRuleConditions: wrap('evaluateRuleConditions', deps, gateway.evaluateRuleConditions.bind(gateway)),
    reserveRuleExecution: wrap('reserveRuleExecution', deps, gateway.reserveRuleExecution.bind(gateway)),
    ...
    loadWebhookDelivery: wrap('loadWebhookDelivery', deps, gateway.loadWebhookDelivery.bind(gateway)),
    deliverWebhook: wrap('deliverWebhook', deps, gateway.deliverWebhook.bind(gateway)),
    recordWebhookAttempt: wrap('recordWebhookAttempt', deps, gateway.recordWebhookAttempt.bind(gateway)),
    disableWebhookEndpoint: wrap('disableWebhookEndpoint', deps, gateway.disableWebhookEndpoint.bind(gateway)),
    deadLetterWebhookDelivery: wrap('deadLetterWebhookDelivery', deps, gateway.deadLetterWebhookDelivery.bind(gateway)),
    ...
    revokeProviderConnection: wrap('revokeProviderConnection', deps, gateway.revokeProviderConnection.bind(gateway)),
    deleteStoredObjects: wrap('deleteStoredObjects', deps, gateway.deleteStoredObjects.bind(gateway)),
