...
Add the Relay MCP server to your Codex configuration as a remote HTTP server
with OAuth, then follow this workflow whenever the task involves social
publishing.

## Configuration

Configure Relay as a remote HTTP MCP server. Authorization is OAuth against
Relay's own issuer; Codex performs the flow. Never place a bearer token in the
