...
own, and it is generated from the real tool list in `src/skills.ts`, so it
cannot document a tool that does not exist.

**These files contain no secret, no token and no platform workaround.** They
also contain no instruction for working around a refusal: if a tool says a
person must confirm, the skill says so and stops.
