...
description: Draft, validate, preview, approve, schedule and publish social posts through Post Array, with receipts and the same approval policy as the app. Use when the user asks to write, review, schedule or publish a social post, or to look at what is scheduled or how a post performed.
---

# Post Array publishing

Use this skill whenever the task involves social publishing. It calls the Post Array
remote MCP server, which enforces the same permissions, approval policy,
idempotency and receipts as the Post Array web app.

## Configuration

Add Post Array as a remote HTTP MCP server and authorize it with `/mcp`. Authorization
is OAuth against Post Array's own issuer. Claude Code performs the flow; you never
